...
phối xuống mọi chỉ mục con (vector, dedup MinHash, knowledge
...
class ArticleStore:
    ...
    - **Lazy** (khi có ``loader``, vd backend Milvus): RAM chỉ là
...
class IndexManager:
    ...
    def __init__(self, settings: Settings | None = None, embedder=None, lazy_init: bool = False) -> None:
        self.settings = settings or Settings.from_env()
        self.embedder = None
        self.vector = None
        self.store = ArticleStore()  # Sẽ được cập nhật sau nếu lazy_init = True
        self.deduper = None
        self.kg = None
        self.generation = 0
        
        if not lazy_init:
            self.load_models(embedder)
            
    def load_models(self, embedder=None) -> None:
        """Nạp các mô hình và tài nguyên (chạy ngầm để không block app)."""
        if self.embedder is not None:
            return  # Đã nạp
            
        _log.info(f"  -> Đang kết nối Vector Database ({self.settings.vector_backend})...")
        if self.settings.embedder == "vi":
            dim = self.settings.vi_embed_dim
        elif self.settings.embedder == "bge":
            dim = self.settings.bge_dim
        else:
            dim = self.settings.embedding_dim

        self.vector = get_vector_index(self.settings, dim=dim)
        
        if embedder is not None:
            self.embedder = embedder
        else:
            _log.info(f"  -> Đang tải mô hình Embedding ({self.settings.embedder}) - Có thể mất thời gian nếu tải lần đầu...")
            self.embedder = get_embedder(self.settings)
        
        # ArticleStore LAZY vì backend Milvus bền vững hỗ trợ get/mget qua metadata
        # -> app khởi động lại không cần nạp lại RAM.
        loader = getattr(self.vector, "get_article", None)
        batch_loader = getattr(self.vector, "mget_articles", None)
        ...
        _log.info("  -> Đang khởi tạo Deduper...")
        self.deduper = get_deduper(self.settings)
        
        _log.info("  -> Đang khởi tạo Knowledge Graph...")
    ...
    def index_article(self, raw: dict | Article) -> Article:
        ...
        try:
            ...
            _log.error(f"  [LỖI] Chuẩn hóa bài viết thất bại: {exc}")
            raise
        ...
        try:
            ...
            _log.error(f"  [LỖI] Tính vector embedding thất bại cho bài {aid}: {exc}")
            raise
        ...
        try:
            ...
            _log.error(f"  [LỖI] Trích xuất thực thể thất bại cho bài {aid}: {exc}")
            raise
        ...
        try:
            ...
            # Sử dụng Hybrid Insert nếu Milvus backend hỗ trợ, nếu không fallback gọi add() thông thường
            if hasattr(self.vector, "add_hybrid"):
                self.vector.add_hybrid(article, vector)
            else:
            ...
            _log.error(f"  [LỖI] Ghi dữ liệu chỉ mục thất bại cho bài {aid}: {exc}")
    ...
    def bulk_index(self, raws: list[dict | Article]) -> int:
        ...
        try:
            ...
            _log.error(f"  [LỖI LÔ] Chuẩn hóa lô bài viết thất bại: {exc}")
            raise
        ...
        try:
            ...
            _log.error(f"  [LỖI LÔ] Tạo vector embedding hàng loạt thất bại: {exc}")
            raise
        ...
        try:
            ...
            _log.error(f"  [LỖI LÔ] Trích xuất thực thể hàng loạt thất bại: {exc}")
            raise
        ...
        try:
            ...
            _log.error(f"  [LỖI LÔ] Ghi các chỉ mục in-memory thất bại: {exc}")
        ...
        try:
            if hasattr(self.vector, "add_batch_hybrid"):
                self.vector.add_batch_hybrid(articles, vectors)
            elif hasattr(self.vector, "add_batch"):
                self.vector.add_batch(article_ids, vectors)
            else:
                for aid, vec in zip(article_ids, vectors):
                    if hasattr(self.vector, "add_hybrid"):
                        self.vector.add_hybrid(articles[article_ids.index(aid)], vec)
                    else:
                        self.vector.add(aid, vec)
        except Exception as exc:
            _log.error(f"  [LỖI LÔ] Ghi lô vector (Milvus/local) thất bại: {exc}")
            for aid in article_ids:
                self._purge(aid)
            raise
    ...
    def remove_article(self, article_id: str) -> None:
        """Gỡ bài khỏi MỌI chỉ mục (F-08) — store, vector, dedup, kg."""
    ...
    def snapshot(self, path: str) -> int:
        ...
        Chỉ lưu Article gốc; các chỉ mục dẫn xuất (vector/dedup/kg) được dựng
